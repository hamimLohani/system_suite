...
network_speed_test() {
  ...
  # Try native tools first based on OS
  case "${OS}" in
    "macOS")
      if command -v networkQuality >/dev/null 2>&1; then
        printf "${COLOR_INFO}Using networkQuality (macOS native tool)...${COLOR_RESET}\n"
        if networkQuality -v 2>/dev/null; then
          pause
          return
        fi
        printf "${COLOR_WARN}networkQuality failed, falling back to curl tests${COLOR_RESET}\n\n"
      fi
      ;;
    "Linux"|"WSL")
      # Try speedtest-cli if available
      if command -v speedtest-cli >/dev/null 2>&1; then
        printf "${COLOR_INFO}Using speedtest-cli...${COLOR_RESET}\n"
        if speedtest-cli --simple 2>/dev/null; then
          pause
          return
        fi
        printf "${COLOR_WARN}speedtest-cli failed, falling back to curl tests${COLOR_RESET}\n\n"
      # Try fast-cli if available
      elif command -v fast >/dev/null 2>&1; then
        printf "${COLOR_INFO}Using fast-cli (Netflix)...${COLOR_RESET}\n"
        if timeout 30 fast --upload 2>/dev/null; then
          pause
          return
        fi
        printf "${COLOR_WARN}fast-cli failed, falling back to curl tests${COLOR_RESET}\n\n"
      fi
      ;;
    "FreeBSD"|"OpenBSD"|"NetBSD")
      # Try speedtest-cli if available on BSD systems
      if command -v speedtest-cli >/dev/null 2>&1; then
        printf "${COLOR_INFO}Using speedtest-cli...${COLOR_RESET}\n"
        if speedtest-cli --simple 2>/dev/null; then
          pause
  ...
  # Fallback to curl-based tests
  if ! command -v curl >/dev/null 2>&1; then
    notify_warn "No speed test tools available. Install speedtest-cli, fast-cli, or curl."
    printf "\n${COLOR_INFO}Installation suggestions:${COLOR_RESET}\n"
    case "${PKG_MANAGER}" in
      "brew") printf "  brew install speedtest-cli\n  npm install -g fast-cli\n" ;;
      "apt") printf "  sudo apt install speedtest-cli curl\n  npm install -g fast-cli\n" ;;
      "dnf"|"yum") printf "  sudo ${PKG_MANAGER} install speedtest-cli curl\n  npm install -g fast-cli\n" ;;
      "pacman") printf "  sudo pacman -S speedtest-cli curl\n  npm install -g fast-cli\n" ;;
      *) printf "  Install speedtest-cli, fast-cli, or curl using your package manager\n" ;;
    esac
    pause
    return
  fi
  
  printf "${COLOR_INFO}Using curl-based speed test...${COLOR_RESET}\n\n"
  
  # Multiple reliable test servers
  local test_files=(
    "1MB:https://proof.ovh.net/files/1Mb.dat:1048576"
    "10MB:https://proof.ovh.net/files/10Mb.dat:10485760"
    "100MB:https://proof.ovh.net/files/100Mb.dat:104857600"
    "1MB:https://speedtest.selectel.ru/1MB:1048576"
    "10MB:https://speedtest.selectel.ru/10MB:10485760"
  )
  
  local fallback_urls=(
    "https://httpbin.org/bytes/1048576:1048576"
  ...
  for test_file in "${test_files[@]}"; do
    ...
    printf "Testing %s... " "${size_name}"
    
    local start_time
    start_time=$(date +%s.%N 2>/dev/null || date +%s)
    
    if curl -L --connect-timeout 10 --max-time 30 -s -o /dev/null "${url}" 2>/dev/null; then
      local end_time
      ...
      if (( $(awk "BEGIN {print (${duration} > 0.1)}") )); then
        local speed_mbps
        speed_mbps=$(awk "BEGIN {printf \"%.1f\", ${file_size} * 8 / ${duration} / 1000000}")
        ...
        printf "${COLOR_SUCCESS}Very Fast${COLOR_RESET}\n"
        ((tests_completed++))
      fi
    else
    ...
    [[ ${tests_completed} -ge 1 ]] && break
  done
  
  # Simple connectivity test if all downloads failed
  if [[ ${tests_completed} -eq 0 ]]; then
    printf "\n${COLOR_INFO}Testing basic connectivity...${COLOR_RESET}\n"
    for fallback in "${fallback_urls[@]}"; do
      IFS=':' read -r url _ <<< "${fallback}"
      printf "Testing %s... " "$(echo "${url}" | sed 's|https\?://||' | cut -d'/' -f1)"
      
      if curl -L --connect-timeout 5 --max-time 10 -s -I "${url}" >/dev/null 2>&1; then
  ...
  if (( $(awk "BEGIN {print (${best_speed} > 0)}") )); then
    printf "Speed: ${COLOR_SUCCESS}%.1f Mbps${COLOR_RESET}\n" "${best_speed}"
  elif [[ ${tests_completed} -gt 0 ]]; then
    printf "${COLOR_SUCCESS}Connection OK${COLOR_RESET} (speed test servers unavailable)\n"
  else
    printf "${COLOR_WARN}No internet connection detected${COLOR_RESET}\n"
  fi
